...
// ordered lunch that day. Sends at 8:10 AM Eastern.
//
// WHY THIS RUNS EVERY ~10 MINUTES INSTEAD OF ONCE A DAY: cron schedules
// run in UTC, but "8:10 AM" in Florida shifts between UTC-5 (winter) and
...
// minutes across a window that covers 8:10 AM Eastern in both cases, and
// the function itself checks the real Eastern-time clock and only
// actually sends once it's genuinely 8:10 AM (or just after) local time.
...
const EASTERN_TARGET_MINUTES = 8 * 60 + 10; // 8:10 AM
...
export default async () => {
  ...
  // Only actually run once it's genuinely 8:10 AM Eastern or just after —
